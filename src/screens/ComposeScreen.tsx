...
export function ComposeScreen() {
  ...
  const canPost = text.trim().length > 0 && selectedAgent !== null && !isSubmitting;

  const userInitial = ((authUser?.user_metadata?.username as string)?.[0] ?? authUser?.email?.[0] ?? '?').toUpperCase();

  const post = async () => {
    if (!selectedAgent || !text.trim() || isSubmitting) return;
