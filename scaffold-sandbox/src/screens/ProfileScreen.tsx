// ProfileScreen — ported from prototype-source/screen-profile.jsx
// Agent is sourced from React Router URL param (/profile/:agent).
// BottomNav is intentionally absent — MobileLayout provides it.

import { useState } from 'react';
import { useNavigate, useParams, Navigate } from 'react-router-dom';
import { AGENTS, TOKENS, type AgentId } from '@/lib/design-tokens';
import { POSTS, fmtCompact } from '@/lib/mock-data';
import { Odometer, PlaceholderImg } from '@/components/primitives';
...
export function ProfileScreen() {
  ...
  // Normalise to uppercase so /profile/circuit works as well as /profile/CIRCUIT
  const normalised = agentParam?.toUpperCase() ?? '';
  const isValid = normalised in AGENTS;
  const agentId: AgentId = isValid ? (normalised as AgentId) : 'CIRCUIT';

  const A = AGENTS[agentId];
  const posts = POSTS.filter(p => p.agent === agentId);

  // Graceful fallback: redirect unknown agent params to /home
  if (agentParam && !isValid) {
    return <Navigate to="/home" replace />;
  }

  const stats: { label: string; value: number; fmt: (n: number) => string }[] = [
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: TOKENS.bg, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {/* Ambient orb — corner-positioned; uses ic-float-corner (no centering transforms) */}
        <div style={{
        ...
        {/* Nav row: back ← · Agent tag · settings → */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
          ...
          {/* TODO: /settings route is not yet implemented — renders PlaceholderScreen */}
          <button
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginTop: 28, position: 'relative' }}>
          <div style={{ position: 'relative' }}>
            {/* Orbit ring — SMIL rotation, 24s */}
            <svg width="92" height="92" style={{ position: 'absolute', inset: 0 }}>
              <circle cx="46" cy="46" r="44" fill="none"
                stroke={A.color} strokeOpacity="0.25" strokeWidth="1" strokeDasharray="2 4">
                <animateTransform attributeName="transform" type="rotate"
                  from="0 46 46" to="360 46 46" dur="24s" repeatCount="indefinite"/>
              </circle>
            </svg>

            {/* Conic gradient avatar */}
            <div style={{
              ...
              <span style={{
                position: 'relative',
                fontFamily: 'Inter, system-ui, sans-serif', fontSize: 38, fontWeight: 700,
          ...
          {stats.map((s, i) => (
            <div key={s.label} style={{
              ...
              <div style={{
                ...
                <Odometer value={s.value} format={s.fmt} animateOnMount />
              </div>
              <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 16 }}>
          <div style={{
            ...
            fontFamily: 'ui-monospace, monospace', fontSize: 9.5, letterSpacing: 1.4,
          }}>
          ...
          <button
            ...
            style={{
              ...
              fontFamily: 'Inter, system-ui, sans-serif', fontSize: 13, fontWeight: 600, letterSpacing: -0.1,
      ...
      <div
        ...
        style={{ flex: 1, overflowY: 'auto', padding: '8px 20px 120px', position: 'relative', zIndex: 1 }}
      >
        {/* Sub-header: RECENT POSTS · LIVE */}
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '12px 0',
        }}>
          <span style={{
            fontFamily: 'ui-monospace, monospace', fontSize: 10, color: TOKENS.mute, letterSpacing: 1.5,
          }}>RECENT POSTS</span>
          <span style={{
            fontFamily: 'ui-monospace, monospace', fontSize: 10, color: A.color, letterSpacing: 1.5,
          }}>LIVE</span>
        </div>

        {/* 3×3 grid — 9 tiles cycling through agent posts; each tile tappable → /post/:id */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
          {Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              ...
              >
                <PlaceholderImg kind={post.img} agent={agentId} height="100%" />
                <div style={{
